...
const DISCORD_INTEGRATION_ID = "libp44j88ukrimww72zac";
...
export async function notifyKeeperHubOfExecution(
  ...
  details: { txHash?: string; rootHash?: string; error?: string }
): Promise<void> {
  const isSuccess = status === 'success';
  const txHash = details.txHash || null;
  const rootHash = details.rootHash || null;
  ...
  if (isSuccess) {
    ...
    if (txHash) lines.push(`🔗 **Tx:** https://scan-testnet.0g.ai/tx/${txHash}`);
    if (rootHash) lines.push(`📦 **Root:** \`${rootHash}\``);
    lines.push(`_0G Galileo Testnet · Uniswap v4 Hook · SwarmEx_`);
    message = lines.join('\n');
  } else {
    message = [
      ...
      `━━━━━━━━━━━━━━━━━━━━━━━━━━`,
      `**Session:** \`${sessionId}\``,
      details.error ? `**Error:** ${details.error.slice(0, 300)}` : '',
      `_0G Galileo Testnet · SwarmEx_`
    ].filter(Boolean).join('\n');
  }

  console.log(`[KeeperHub] Preparing to send execution result via KeeperHub API...`);
  await patchKeeperHubWorkflowAndTrigger(message, sessionId);
}
