...
async function patchKeeperHubWorkflowAndTrigger(message: string, sessionId: string): Promise<boolean> {
  ...
  const patchPayload = {
    nodes: [
      ...
      { id: "discord-to-swarm-action", type: "action", data: { type: "action", label: "Discord-to-Swarm Execute", config: { actionType: "HTTP Request", endpoint: `${RENDER_URL}/orchestrator/run`, httpMethod: "POST", httpHeaders: '{"Content-Type":"application/json"}', httpBody: '{"intent": "{{@discord-trigger:Discord Command Trigger.content}}", "sessionId": "discord-{{@discord-trigger:Discord Command Trigger.author_id}}"}' } }, position: { x: 400, y: 600 } },
      { id: "discord-ack", type: "action", data: { type: "action", label: "Discord Acknowledgment", config: { actionType: "discord/send-message", integrationId: DISCORD_INTEGRATION_ID, discordMessage: "🚀 **SwarmEx Session Started**\nAnalyzing intent: `{{@discord-trigger:Discord Command Trigger.content}}`" }, status: "idle" }, position: { x: 400, y: 800 } }
    ],
    edges: [
