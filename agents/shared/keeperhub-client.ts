...
async function patchKeeperHubWorkflowAndTrigger(message: string, sessionId: string): Promise<boolean> {
  ...
  const patchPayload = {
    nodes: [
      ...
      { id: "discord-trigger", type: "trigger", data: { type: "trigger", label: "Discord Command Trigger", config: { triggerType: "discord/on-message", integrationId: DISCORD_INTEGRATION_ID }, status: "idle" }, position: { x: 100, y: 600 } },
      { id: "discord-to-swarm-action", type: "action", data: { type: "action", label: "Discord-to-Swarm Execute", config: { actionType: "HTTP Request", endpoint: `${RENDER_URL}/orchestrator/run`, httpMethod: "POST", httpHeaders: '{"Content-Type":"application/json"}', httpBody: '{"intent": "{{@discord-trigger:Discord Command Trigger.message}}", "sessionId": "discord-{{@discord-trigger:Discord Command Trigger.authorId}}"}' } }, position: { x: 400, y: 600 } },
      { id: "discord-ack", type: "action", data: { type: "action", label: "Discord Acknowledgment", config: { actionType: "discord/send-message", integrationId: DISCORD_INTEGRATION_ID, discordMessage: "🚀 **SwarmEx Session Started**\nAnalyzing intent: `{{@discord-trigger:Discord Command Trigger.message}}`" }, status: "idle" }, position: { x: 400, y: 800 } }
    ],
    edges: [
